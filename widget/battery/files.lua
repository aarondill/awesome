...
local PATH_TO_ICONS = path.resolve(source_path.dirname(1), "icons")
...
local function get_battery_info(battery_path, callback_fn)
  parallel_async(keys, function(key, done)
    ...
    read_async(path.join(battery_path, battery_file.path), function(content, _)
...
local function find_battery_path(cb)
  ...
  list_directory(power_supply_dir, { match = "BAT.+", max = 1 }, function(files)
    ...
    return cb(path.join(power_supply_dir, bat))
  end)
end
...
local function get_icon(name, ext)
  ext = ext or ".svg"
  return path.join(PATH_TO_ICONS, name .. ext)
end
