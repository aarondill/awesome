...
---@field path string? the /sys/class/backlight/ folder containing the brightness information
local defaults = {
  step = 5,
  min = 1,
  max = 100,
...
function bcontrol.new(args)
  ...
  assertions.type(config.path, { "string", "nil" }, "path")
  assert(config.min >= 0, "min must be >= 0")
  assert(config.max <= 100, "max must be <= 100")
  assert(config.step >= 0 and config.step <= 100, "step must be >= 0 and <= 100")

  local self = tables.clone(bcontrol)
  self.config = config
...
function bcontrol:get_max_bright(callback)
  ...
  return read_async(path.join(self.config.path, "max_brightness"), function(max_bright)
...
function bcontrol:get(callback)
  if not self.config.path then return end
  return read_async(path.join(self.config.path, "brightness"), function(bright)
    if not bright then return end
    return self:get_max_bright(function(max_bright) return calc_bright(bright, max_bright, callback) end)
  end)
end
...
function bcontrol:set(brightness, callback)
  if not self.config.path then return end
  brightness = constrain(brightness, self.config.min, self.config.max)
  self:get_max_bright(function(max_bright)
    local brightness_val = math.floor((brightness / 100) * max_bright + 0.5)
    write_async(path.join(self.config.path, "brightness"), tostring(brightness_val), function(err)
...
function bcontrol:up(step, callback)
  local s = step or self.config.step
  return self:get(function(v) return self:set(v + s, callback) end)
end
...
return setmetatable(bcontrol, { __call = function(_, ...) return bcontrol.new(...) end })
