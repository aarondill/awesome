...
local function query_file_type_async(path, cb, types)
  ...
  return new_file_for_path(path):query_info_async(attr, 0, 0, nil, function(file2, task)
    local info, err = file2:query_info_finish(task)
    if not info then return cb(nil, err, path) end
    local ftype = info:get_file_type()
    ---@type GFileType
    local t = assert(Gio.FileType[ftype], "Invalid return from get_file_type: " .. tostring(ftype))
    return cb(t, nil, path)
  end)
end
