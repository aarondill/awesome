...
local stdout = process:get_stdout_pipe()
assert(stdout, "Could not get stdout pipe")
local input = Gio.DataInputStream.new(stdout)
local fline = input:read_line()
if fline then
  notifs.normal("Updating git submodules")
  notifs.normal(tostring(fline))
end
-- Give a progress notification on each line
while true do
  local line = input:read_line()
  if not line then break end
  notifs.normal(tostring(line))
end
